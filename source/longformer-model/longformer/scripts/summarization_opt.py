...
class SummarizationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        input_ids = self.tokenizer.encode(entry['sentence1_key'], truncation=True, max_length=self.max_input_len)
        output_ids = self.tokenizer.encode(entry['label'], truncation=True, max_length=self.max_output_len)
...
class Summarizer(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_nb):
        ...
        return {'vloss': vloss,
                ...
                'acc': vloss.new_zeros(1) + acc}

    def validation_epoch_end(self, outputs):
        for p in self.model.parameters():
            p.requires_grad = True

        names = ['vloss', 'rouge1', 'rouge2', 'rougeL', 'rougeLsum', 'acc']
        metrics = []
        for name in names:
    ...
    def add_model_specific_args(parser, root_dir):
        ...
        parser.add_argument("--train_file", type=str, default=None, help="train file path")
        parser.add_argument("--validation_file", type=str, default=None, help="validation file path")
        parser.add_argument("--test_file", type=str, default=None, help="test file path")
...
def main(args):
    ...
    if args.dataset_name is None:
        ...
        model.hf_datasets = datasets.load_dataset('scientific_papers', 'arxiv')
        ...
        model.hf_datasets = datasets.load_dataset(extension, data_files=data_files)
